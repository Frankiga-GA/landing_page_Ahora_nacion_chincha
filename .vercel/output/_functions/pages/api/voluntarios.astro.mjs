...
const POST = async ({ request }) => {
  try {
    ...
    try {
      ...
      verifyJson = await verify.json();
    } catch (error2) {
      console.error("Turnstile siteverify error:", error2);
    ...
    console.error("POST /api/voluntarios crash:", error);
    return json(500, { error: "Error inesperado. Inténtalo nuevamente." });
  }
};
